...
export async function POST(req: NextRequest) {
  try {
    ...
    const body = await req.json() as { title?: unknown; studentId?: unknown };
    ...
    await db.collection("users").doc(hubUid).collection("tasks").add({
      title,
      done: false,
      source: "parent_assigned",
      createdAt: FieldValue.serverTimestamp(),
    });

    return NextResponse.json({ ok: true });
  } catch (err) {
    console.error("[parent/add-task]", err);
